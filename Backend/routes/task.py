...
async def add_comment(
    ...
    if comment.is_admin_comment and current_user.employee_role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can add admin comments"
        )
...
async def remove_comment(
    ...
    # Only admins can delete admin comments
    if comment.is_admin_comment and current_user.employee_role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can delete admin comments"
        )
...
async def update_comment(
    ...
    # Only admins can update admin comments
    if comment.is_admin_comment and current_user.employee_role != "admin":
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can update admin comments"
        )
